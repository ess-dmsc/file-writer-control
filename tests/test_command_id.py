...
def test_command_id():
    ...
    used_re = re.compile(
        r"(?P<coding>(?P<host>.*)-(?P<pid>\d+)-(?P<cmd>.*)-(?P<time>[A-F0-9]{8})-(?:[A-F0-9]{4})-)(?P<checksum>[A-F0-9]{8})"
    )
    ...
    assert (
        fabs(time.time() - (int("0x" + match_res.group("time"), 0) ^ 0xFFFFFFFF)) <= 1.0
    )
    assert "{:8X}".format(
        adler32(match_res.group("coding").encode())
    ) == match_res.group("checksum")
